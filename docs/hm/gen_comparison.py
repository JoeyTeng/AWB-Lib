...
from awblib.hm import balance
from awblib.utils import load_image, visualise_histogram

tilted = [chr(ord('a') + i) for i in range(6)]
prefix = "data/"
suffix = ".png"
...
for stem in tilted:
    fig, axs = plt.subplots(ncols=3, nrows=3, figsize=(18, 20))
    ...
    table = table_axes.table(
        ...
        colLabels=["Tilted", "Fixed (paper)", "Fixed (ours)"],
        loc="center",
    )
    table.scale(8, 4)
    ...
    fig.suptitle("tilted vs fixed (paper) vs fixed (ours);"
